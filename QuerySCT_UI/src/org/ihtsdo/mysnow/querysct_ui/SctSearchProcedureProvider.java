...
public class SctSearchProcedureProvider implements SearchProvider {
    private static QuerySCT querysct = Lookup.getDefault().lookup(QuerySCT.class);
    private static final long ROOT_ID = 71388002L;
    
    @Override
    public void evaluate(SearchRequest request, SearchResponse response) {
         if(request.getText().length()>2){
        Node rootNode = querysct.getNodebyID(ROOT_ID);
        if (rootNode == null) {
            return;
        }
        Collection<Node> allsctnodes = querysct.getAllSctNodes(rootNode);
        for (Node sctnode:allsctnodes){
//          for (Iterator<Node> iterator =querysct.getAllSctNodeIterator(71388002); iterator.hasNext();){
